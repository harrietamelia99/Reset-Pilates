      ...
      <div className="mx-auto max-w-6xl px-4 md:px-6">
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          ...
          <Link
            ...
            className="inline-flex shrink-0 items-center gap-2 self-start border border-charcoal bg-charcoal px-6 py-3 text-xs font-bold uppercase tracking-wide text-white transition hover:opacity-90 md:self-auto"
          >
        ...
        <ul className="mt-10 grid gap-4 sm:grid-cols-3 sm:gap-5">
          {INSTAGRAM_HOME_GRID.map((item) => (
            <li key={item.src} className="relative aspect-square overflow-hidden border border-charcoal/10 bg-light-grey/20">
              <Image
                src={item.src}
                alt={item.alt}
                fill
